...
public class Generator extends OpenBrowser {
	...
	public static void gerar(String Cenario) {
		...
		try {
			PdfWriter.getInstance(document, new FileOutputStream("src/test/resources/PDF/" + Cenario + ".pdf"));
			document.open();
			document.newPage();
	...
	public static void TextData() {
		...
		Generator.addText("Endereço IP: " + InfraUtils.getIpClient());
		Generator.addText("Usuário local ou de rede: " + InfraUtils.getWindowsUser());
		Generator.addText("Nome do computador de execução: " + InfraUtils.getPcName());
	}
